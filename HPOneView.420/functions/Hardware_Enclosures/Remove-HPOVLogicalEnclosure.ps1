	...
	End
	{
		...
		ForEach ($_enclosure in $_LECollection)
		{
			...
			if ($PSCmdlet.ShouldProcess($_enclosure.name,"Remove Logical Enclosure from appliance '$($_enclosure.ApplianceConnection.Name)'")) 
			{
				...
				"[{0}] Removing Logical Enclosure '$($_enclosure.name)' from appliance '$($_enclosure.ApplianceConnection.Name)'." -f $MyInvocation.InvocationName.ToString().ToUpper() | Write-Verbose
